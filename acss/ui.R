shinyUI(fluidPage(
  ...
  #withMathJax(),
  sidebarLayout(
    sidebarPanel(strong("Strings to evaluate"),
                 br(),
                 p("Strings must be of length < 12 and separated by blank spaces."), 
                 
                 tags$textarea(id="string", 
                               label="test", 
                               rows=3, 
                               cols=40, 
                               "AAAAAAAAAA ATATATATAT ATGCCGGCCT"),
                 
                 radioButtons("alphabet",
                              label = "Alphabet size (number of possible symbols)", 
                              inline = TRUE,
